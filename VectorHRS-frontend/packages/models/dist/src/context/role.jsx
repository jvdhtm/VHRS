import { role_list, role_create, role_read, role_update, role_partial_update, role_delete, } from "../api";
...
/* prettier-ignore */
const RoleContext = createContext(defaultContextState);
export const RoleProvider = ({ children, headers }) => {
    /* prettier-ignore */
    const [RoleDataList, setRoleDataList] = useState(initialState);
    /* prettier-ignore */
    ...
    const roleList = async (data) => {
        if (data) {
            ...
            let newRole = prevStateResults.map((el) => {
    ...
    const roleUpdate = async (id, data) => {
        if (id && data) {
            ...
            let newRole;
            if (!Array.isArray(result.data))
                newRole = prevStateResults.map((el) => el.id === result.data.id ? { ...el, ...result.data } : el);
            //update bulk
            else
                newRole = prevStateResults.map((el) => el.id === result.data.id ? { ...el, ...result.data } : el);
            setRoleDataList({
    ...
    const rolePartial = async (id, data) => {
        if (id && data) {
            setLoading(true);
    ...
    const roleDelete = async (id) => {
        if (id) {
            ...
            //delete
            const newRole = prevStateResults.filter((el) => el.id !== id);
            setRoleDataList({
                ...RoleDataList,
                results: newRole,
            });
